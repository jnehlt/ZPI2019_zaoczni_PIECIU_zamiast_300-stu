pipeline {
    ...
    stages {
        ...
        stage('build project') {
            steps {
                sh '''
                    docker run --rm -v "$(pwd)":/workdir -w /workdir --cpus="0.4" --memory="500m" thyrlian/android-sdk ./gradlew clean -Dkotlin.compiler.execution.strategy=in-process -Dkotlin.incremental=false
                '''
            }
        }
    }
